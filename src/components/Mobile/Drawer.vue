    ...
    <div>
        ...
        <div class="offcanvas_menu">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <div class="offcanvas_menu_wrapper" :class="[sidebar ? 'active' : '']" style="overflow-y: scroll; overflow-x: hidden">
                            ...
                            <div id="menu" :style="heightPro" class="text-left">
                                <ul class="offcanvas_main_menu list-unstyled">
                                    <li v-for="(item, index) in menuItems" :key="index" class="menu-item-has-children active" :class="[menuOpen === item.linkText ? 'menu-open' : '']">
                                        <span v-if="item.submenu && item.submenu.length > 0 && nasted" class="menu-expand"><i class="fa fa-angle-down"></i></span>
                                        <router-link style="text-transform: capitalize" v-if="item.link" :to="item.link">
                                            {{ item.linkText }}</router-link>
                                        <a v-else href="#" @click.prevent="show(item.linkText)">
                                            {{ item.linkText }}</a>
                                        <ul v-if="item.submenu && nasted" :id="item.linkText" class="sidebar-sub-menu list-unstyled ml-3" :class="[menuOpen === item.linkText ? 'expend_menu_item' : '',]">
                            ...
                            <div class="footer-widget-info">
                                <ul>
                                    ...
                                    <li>
                                        <a href="#"><i class="fal fa-map-marker-alt"></i> 442 Belle Terre St Floor 7, San Francisco, AV 4206</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
